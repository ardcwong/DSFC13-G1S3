...
def process_keyword(keyword, df):
    ...
    similarities = {area: compute_similarity(keyword_synsets, synsets) for area, synsets in focus_area_synsets.items()}
    best_match_focus_area = max(similarities, key=similarities.get)
    
    st.session_state['focus_area'] = best_match_focus_area
